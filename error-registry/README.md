...
related_skills: [architecture-constraints, task-progress, event-report]
---
...
| `STATE_CHECK_MISSING` | 实现项目前未实时查看仓库状态 — 必须 `bash scripts/check-preflight.sh` 后再动手 |
...
| `SOLO_EXECUTION_OVERRIDE` | Guidance Agent 独自执行多步骤/多文件项目，未走 Agent Team 管道 — 必须走 10.2 管道，用户说"全部"是 scope 不是执行模式 |
| `GITEE_PATCH_NEEDS_NAME` | Gitee PATCH /repos/{owner}/{repo} 返回 400 "name is missing" — body 必须包含 `"name":"repo-name"` 参数 |
| `CLAUDE_DEEPSEEK_CONFIG` | Claude Code 配 DeepSeek 需用 `ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic` + `ANTHROPIC_AUTH_TOKEN`（非 ANTHROPIC_API_KEY）— 跳过 onboarding 需 `~/.claude/settings.json` 设 `hasCompletedOnboarding: true` |
| `XUEXITONG_UEDITOR_UPLOAD` | 学习通 UEditor 文件上传触发浏览器原生文件对话框，无法通过 browser DOM 自动化 — 需用户手动选择文件或使用 UEditor API `editor.setContent()` 填入文本 |
