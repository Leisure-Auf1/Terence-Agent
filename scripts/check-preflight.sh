...
# 扫描仓库的常见 PII 模式
PII_FOUND=0
for p in "projects/*/task-progress" "event-report" ".hermes" "agent-team"; do
    for f in $(find "$REPO_DIR/$p" -name "*.md" -o -name "*.json" -o -name "*.yaml" -o -name "*.sh" 2>/dev/null | head -50); do
        if grep -qE "[一-龥]{2,4}/[0-9]{8}" "$f" 2>/dev/null; then
            echo "⚠️  $f — 可能包含中文姓名+学号"
...
if [ "$PII_FOUND" -gt 0 ]; then
    echo "❌ 发现 $PII_FOUND 处可能的 PII 残留！立即清理 → 记入 error-registry PII_LEAK"
else
    echo "✅ 未发现明显 PII 残留"
fi
